...
my $mark="right";
GetOptions(
		"cut:n"=>\$cutoff,     ## the largest distance of a pair X or Y axis
...
sub is_pair{
	...
	my @aa=split(/\t/,$line1);
	my @bb=split(/\t/,$line2);
	$is_pair=1 if(abs($aa[1]-$bb[1])<$cutoff && abs($aa[4]-$bb[4])<$cutoff);	
	$is_pair=1 if(abs($aa[2]-$bb[1])<$cutoff && abs($aa[5]-$bb[4])<$cutoff);
	$is_pair=1 if(abs($aa[2]-$bb[2])<$cutoff && abs($aa[5]-$bb[5])<$cutoff);
	$is_pair=1 if(abs($aa[1]-$bb[2])<$cutoff && abs($aa[4]-$bb[5])<$cutoff);		
	return $is_pair;
}
